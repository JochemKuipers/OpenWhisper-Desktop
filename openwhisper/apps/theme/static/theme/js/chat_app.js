import { apiFetch, bootstrapSessionJwt, getAccess } from "./chat_app/api.js";
import {
	closeDialog,
	isDialogOpen,
	openDialog,
...
function displayChatTitle(chat) {
	if (chat?.display_title) {
		return String(chat.display_title);
	}
	return "Chat";
}

function chatMemberSubtitle(chat) {
	if (chat && chat.member_subtitle != null) {
		return String(chat.member_subtitle);
	}
	return "";
}
...
function syncChatHeader(chat) {
	...
	if (els.btnRenameChat) {
		els.btnRenameChat.classList.toggle("hidden", !isAdmin || memberCount < 2);
	}
...
function renderChatMembersPanel() {
	...
	const chat = chatsCache.find(
		(c) => chatPkFromUrl(c.url) === String(selectedChatPk),
	);
	if (!chat) return;
	...
	users.forEach((u) => {
		...
		span.textContent = uname + (uname === currentUsername ? " (you)" : "");
		...
		li.appendChild(left);
		const canRemove =
			!!chat.is_admin && uname !== currentUsername && uname !== adminName;
		if (canRemove) {
			...
			btn.addEventListener("click", () => {
				...
				removeChatMember(uname).catch((err) => {
...
function lastPreview(chat) {
	...
	return (
		(last.sender?.username ? `${last.sender.username}: ` : "") +
...
function renderConversationList(filterText) {
	...
	list.forEach((chat) => {
		...
		const lastMsg = chat.messages?.length
			...
			: null;
		const when = lastMsg?.created_at ? formatShortTime(lastMsg.created_at) : "";
		li.innerHTML =
...
function rejectAllSocialPending(message) {
	Object.keys(pendingSocialAcks).forEach((k) => {
...
function applySocialNotification(j) {
	...
	if (t === "friend_request_received") {
		...
		const pk = String(j.chat_id);
		const cached = chatsCache.find((c) => chatPkFromUrl(c.url) === pk);
		if (cached && j.display_title != null) {
			cached.display_title = j.display_title;
			if (j.member_subtitle != null) {
				cached.member_subtitle = j.member_subtitle;
			}
			renderConversationList(els.convSearch ? els.convSearch.value : "");
			if (selectedChatPk && String(selectedChatPk) === pk) {
				syncChatHeader(cached);
			}
		}
		loadChats()
			.then(() => {
				...
				const ch = chatsCache.find(
					(c) => chatPkFromUrl(c.url) === String(j.chat_id),
				);
				if (ch) syncChatHeader(ch);
				if (els.chatMembersModal && isDialogOpen(els.chatMembersModal)) {
	...
	if (els.peopleModal && isDialogOpen(els.peopleModal)) {
		refreshPeoplePanels();
...
function socialRpc(action, fields) {
	return ensureSocialWsConnected().then(
		() =>
		...
		try {
			...
			if (
				...
				loadChats()
					.then(() => {
						const ch = chatsCache.find(
							(c) => chatPkFromUrl(c.url) === String(chatPk),
						);
						if (ch) syncChatHeader(ch);
						if (els.chatMembersModal && isDialogOpen(els.chatMembersModal)) {
							renderChatMembersPanel();
			...
			if (j.type !== "message.created" || String(j.chat_id) !== String(chatPk))
				return;
...
function renderMessages(_chatPk, msgs) {
	els.thread.innerHTML = "";
	const chronological = (msgs || [])
		.slice()
		.sort((a, b) => new Date(a.created_at) - new Date(b.created_at));
	chronological.forEach((m) => {
		const uname = m.sender?.username ? m.sender.username : "?";
...
async function selectChat(pk) {
	...
	const chat = chatsCache.find((c) => chatPkFromUrl(c.url) === pk);
	if (!chat) return;
...
function initAttachmentPicker() {
	...
	inp.addEventListener("change", () => {
		...
		uploadChatAttachments(files).catch((err) => {
...
async function loadFriendsUsernames() {
	...
	friendsUsernames = new Set((rows || []).map((r) => r.username));
}
...
async function loadFriendRequests() {
	...
	incomingFromUsernames = new Set((data.incoming || []).map((r) => r.username));
	outgoingToUsernames = new Set((data.outgoing || []).map((r) => r.username));
}
			...
			(actions) => {
				actions.appendChild(
					createActionButton(
						...
						() => {
							acceptFriendRequest(uname).catch((err) => {
				...
				actions.appendChild(
					createActionButton(
						...
						() => {
							cancelFriendRequest(uname).catch((err) => {
...
function removeFriend(username) {
	...
	socialRpc("friend_remove", { username: username }).catch((err) => {
...
function renderPeopleSearchRows(rows) {
	...
	rows.forEach((row) => {
		...
		if (!isFriend && incoming) {
			...
			lbl.className =
				"rounded-lg bg-slate-100 px-2 py-1 text-xs text-slate-600";
			...
			btnReq.addEventListener("click", () => {
				sendFriendRequest(uname).catch((err) => {
...
function openRenameChatModal() {
	...
	if (!chat?.is_admin || (chat.users || []).length < 2) return;
	const stored =
...
async function saveRenameChatTitle() {
	...
	try {
		...
		await loadChats();
		const ch = chatsCache.find((c) => chatPkFromUrl(c.url) === String(pk));
		if (ch) syncChatHeader(ch);
	} catch (e) {
...
async function openInviteModal() {
	...
	const inChat = new Set((chat.users || []).map((u) => u.username));
	const eligible = (rows || []).filter((r) => !inChat.has(r.username));
	...
	eligible.forEach((row) => {
		...
		btn.addEventListener("click", () => {
			inviteFriendToChat(uname).catch((err) => {
...
async function inviteFriendToChat(username) {
	const pk = selectedChatPk;
	if (!pk) return;
	await apiFetch(`/api/chats/${encodeURIComponent(pk)}/invite/`, {
		method: "POST",
		body: { username: username },
	});
	closeInviteModal();
...
if (els.btnInviteChat)
	els.btnInviteChat.addEventListener("click", () => {
		openInviteModal().catch(() => {});
	});
if (els.btnRenameChat)
	els.btnRenameChat.addEventListener("click", () => {
		openRenameChatModal();
	});
if (els.renameChatSave)
	els.renameChatSave.addEventListener("click", () => {
		saveRenameChatTitle().catch(() => {});
	});

if (els.btnChatMembers)
	els.btnChatMembers.addEventListener("click", () => {
		openChatMembersModal().catch(() => {});
	});
